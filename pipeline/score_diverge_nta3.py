#!/usr/bin/env python3
"""Score the frozen NTA3 full-document transfer gate."""
...
def score(report: dict[str, object]) -> dict[str, object]:
    if report.get("schema") != "shohin-diverge-nta3-evaluation-v1" or report.get("rows") != 279:
        raise ValueError("NTA3 evaluation contract differs")
    if report.get("updates_after_fta1") != 0:
        raise ValueError("NTA3 is a zero-update gate")
    scanner = report["scanner"]
    compiler = report["compiler"]
    arms = report["arms"]
    normal = arms["normal"]["counts"]
    conditions = {
        "scanner_rows_exact": scanner["exact_rows"] == scanner["target_rows"] == 279,
        "scanner_transactions_exact": scanner["exact_transactions"] == scanner["target_transactions"] == 963,
    ...
    return {
        "schema": "shohin-diverge-nta3-gate-v1",
        ...
        "decision": (
            "freeze_transaction_architecture_and_design_broader_typed_operation_learning"
            if passed
            else "close_full_document_scanner_before_broader_operations"
        ),
    }

#!/usr/bin/env python3
"""Reduce the two frozen CTE1 development evaluations conjunctively."""
...
SCHEMA = "shohin-cte1-development-aggregate-v1"


class CTE1AggregateError(ValueError):
    """Frozen CTE1 reports differ."""
...
def load(path: Path, control: str) -> dict:
    ...
    if (
        report.get("schema") != "shohin-cte1-development-evaluation-v1"
        ...
        raise CTE1AggregateError(f"{control} report differs")
    return report
...
def run(args: argparse.Namespace) -> dict:
    ...
    for key in (
        ...
        if normal.get(key) != shuffled.get(key):
            raise CTE1AggregateError(f"report custody differs: {key}")
    n = normal["counts"]
    s = shuffled["counts"]
    ...
    gates = {
        ...
        "aligned_beats_direct_by_33": correct - 267 >= 33,
        ...
        "public_test_closed": normal.get("public_test_opened") is False
        and shuffled.get("public_test_opened") is False,
    }
    result = {
        "schema": SCHEMA,
    ...
    if args.output.exists():
        raise CTE1AggregateError("refusing existing aggregate")

#!/usr/bin/env python3
"""Build the frozen source-disjoint DIVERGE-EIC1 confirmation board."""
...
from diverge_eic1_confirmation_data import (
    BOARD_ROWS,
    BOARD_SEED,
    MODES,
    NAMES,
...
def main() -> None:
    ...
    if args.output.exists() or args.seed != BOARD_SEED:
        raise SystemExit("EIC1 confirmation output or seed differs")
    if len(args.prior_board) != len(args.prior_board_sha256):
        raise SystemExit("EIC1 prior-board receipts differ")
    if sha256_path(args.cgl1_public) != args.cgl1_public_sha256:
        raise SystemExit("EIC1 public training hash differs")
    ...
    for path, expected in zip(args.prior_board, args.prior_board_sha256, strict=True):
        if sha256_path(path) != expected:
            raise SystemExit(f"EIC1 prior board hash differs: {path}")
    ...
    if any(overlap.values()) or len(identities) != BOARD_ROWS:
        raise SystemExit(f"EIC1 confirmation overlap differs: {overlap}")
    ...
    if renderer_order != Counter(
        ...
        raise SystemExit("EIC1 confirmation renderer/order receipt differs")

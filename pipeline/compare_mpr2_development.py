#!/usr/bin/env python3
"""Apply the frozen conjunctive MPR2 practical temporal-revision gate."""
...
OUTPUT_SCHEMA = "shohin-mpr2-development-comparison-v1"


class MPR2ComparisonError(RuntimeError):
    """MPR2 result receipts do not satisfy the frozen comparison contract."""
...
def load(path: Path) -> dict[str, Any]:
    ...
    if value.get("status") != "complete":
        raise MPR2ComparisonError(f"incomplete MPR2 input: {path}")
    return value
...
def domains(report: dict[str, Any]) -> dict[str, int]:
    return {task: int(report["metrics"][task]["generated_correct"]) for task in ("math500", "bbh_logic", "mbpp")}


def atomic_json(path: Path, payload: dict[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        raise MPR2ComparisonError(f"refusing existing comparison: {path}")
...
def compare(args: argparse.Namespace) -> dict[str, Any]:
    evaluations = {name: load(getattr(args, f"{name}_report")) for name in ("aligned", "shuffled", "hidden", "owner")}
    if any(report.get("schema") != EVAL_SCHEMA or report.get("split") != "development" or int(report.get("full_row_count", -1)) != 1289 for report in evaluations.values()):
        raise MPR2ComparisonError("MPR2 evaluation geometry differs")
    fits = {name: load(getattr(args, f"{name}_fit")) for name in ("aligned", "shuffled", "hidden")}
    data = load(args.data_report)
    semantic = load(args.semantic_attribution)
    if data.get("schema") != DATA_SCHEMA or data.get("holdout_used") is not False or data.get("complete_retention") is not True or semantic.get("schema") != SEMANTIC_SCHEMA:
        raise MPR2ComparisonError("MPR2 data or semantic receipt differs")
    expected_data = {"aligned": data["outputs"]["train_aligned"]["sha256"], "shuffled": data["outputs"]["train_shuffled"]["sha256"], "hidden": data["outputs"]["train_aligned"]["sha256"]}
    for arm, fit in fits.items():
        ...
        if (
            ...
            raise MPR2ComparisonError(f"MPR2 {arm} fit differs")
    ...
    semantic_net = int(counts.get("remaining_possible_semantic_repairs", -1)) - int(counts.get("strict_breaks", -1))
    gates = {
        "aligned_at_least_286": scores["aligned"] >= 286,
    ...
    payload = {
        ...
        "decision": "open_one_sealed_holdout" if gate_pass else "close_exact_mpr2",
        "inputs": {name: sha256_file(path) for name, path in vars(args).items() if isinstance(path, Path) and path.exists()},
    }
...
def main() -> int:
    ...
    for name in ("aligned_report", "shuffled_report", "hidden_report", "owner_report", "aligned_fit", "shuffled_fit", "hidden_fit", "data_report", "semantic_attribution"):

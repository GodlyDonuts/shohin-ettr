#!/usr/bin/env python3
"""Apply the frozen matched CFR1 development gate."""
...
SCHEMA = "shohin-cfr1-development-comparison-v1"
EVAL_SCHEMA = "shohin-idr1-revision-evaluation-v1"
FIT_SCHEMA = "shohin-hf-product-reasoning-training-v1"


class CFR1ComparisonError(RuntimeError):
    """CFR1 fit or development evidence differs from the frozen contract."""
...
def load(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or value.get("status") != "complete":
        raise CFR1ComparisonError(f"incomplete report: {path}")
    return value
...
def score(report: dict[str, Any], domain: str = "overall") -> tuple[int, int]:
    ...
    if not isinstance(value, dict) or int(value.get("total", 0)) <= 0:
        raise CFR1ComparisonError(f"missing score for {domain}")
...
def atomic_json(path: Path, value: dict[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        raise CFR1ComparisonError(f"refusing existing comparison: {path}")
...
def compare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not complete_eval(aligned) or not complete_eval(shuffled):
        raise CFR1ComparisonError("CFR1 evaluation coverage differs")
    for key in ("model_root", "model_revision", "data_sha256", "data_report_sha256"):
        if aligned.get(key) != shuffled.get(key):
            raise CFR1ComparisonError(f"CFR1 evaluation setting differs: {key}")
    ...
    if not complete_fit(aligned_fit, aligned_data) or not complete_fit(
        shuffled_fit, shuffled_data
    ):
        raise CFR1ComparisonError("CFR1 fit receipt differs")
    for key in (
        "model_root",
        "model_revision",
        "model_loader",
    ...
    result = {
        ...
        "aligned_minus_shuffled_points": 100.0
        * (aligned_correct - shuffled_correct)
        / total,
        ...
        "decision": (
            "allow_one_sealed_cfr1_holdout_evaluation"
            if gate_pass
            else "close_exact_cfr1_without_rescue"
        ),
    }

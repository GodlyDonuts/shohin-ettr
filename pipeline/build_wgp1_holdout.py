...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for line in args.source.read_text().splitlines():
        ...
        if source.get("split") != args.expected_split or family not in FAMILIES:
            continue
        ...
        try:
            ...
            identity = hashlib.sha256(
                f"shohin-wgp1-holdout-v1\0{question_hash}".encode()
            ).hexdigest()
            pstc = {
...
def main() -> int:
    ...
    parser.add_argument(
        "--expected-split",
        choices=("eval_seed", "confirmation_seed"),
        default="eval_seed",
    )

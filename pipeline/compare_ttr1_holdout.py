#!/usr/bin/env python3
"""Apply the frozen TTR1 holdout gate to complete matched reports."""
...
SCHEMA = "shohin-ttr1-holdout-comparison-v1"
...
class TTR1HoldoutComparisonError(RuntimeError):
    """A TTR1 holdout report set is incomplete or not causally matched."""
...
def load_report(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or value.get("status") != "complete":
        raise TTR1HoldoutComparisonError(f"incomplete TTR1 report: {path}")
    return value
...
def accuracy(report: dict[str, Any], domain: str = "overall") -> float:
    ...
    if not isinstance(metrics, dict) or metrics.get("total", 0) <= 0:
        raise TTR1HoldoutComparisonError(f"missing TTR1 metrics: {domain}")
...
def correct(report: dict[str, Any], domain: str) -> int:
    ...
    if not isinstance(metrics, dict):
        raise TTR1HoldoutComparisonError(f"missing TTR1 domain: {domain}")
...
def compare(args: argparse.Namespace) -> dict[str, Any]:
    development = load_report(args.development_comparison)
    if (
        development.get("schema") != "shohin-ttr1-development-comparison-v1"
        or not development.get("gate_pass")
        or not development.get("holdout_authorized")
    ):
        raise TTR1HoldoutComparisonError("development did not authorize holdout")
    ...
    if treatment.get("schema") != "shohin-idr1-revision-evaluation-v1":
        raise TTR1HoldoutComparisonError("treatment report schema differs")
    shared = ("split", "model_revision", "data_sha256", "data_report_sha256")
    if treatment.get("split") != "holdout":
        raise TTR1HoldoutComparisonError("TTR1 holdout comparison opened another split")
    for name, report in controls.items():
        if (
            ...
            or report.get("full_row_count") != 1279
        ):
            raise TTR1HoldoutComparisonError(
                f"TTR1 holdout control is not matched: {name}"
            )
    ...
    gates = {
        ...
        "complete_identity_coverage": all(
            report.get("full_row_count") == 1279
    ...
    result = {
        ...
        "development_comparison": {
            "path": str(args.development_comparison.resolve()),
            "sha256": sha256_file(args.development_comparison),
        },
        ...
        * (treatment_accuracy - control_accuracy[strongest_name]),
        ...
        "product_authorized": all(gates.values()),
    }
...
def main() -> int:
    ...
    parser.add_argument("--development-comparison", type=Path, required=True)

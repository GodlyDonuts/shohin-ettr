#!/usr/bin/env python3
"""Fail-closed reducer for the frozen FTA1 component gate."""
...
class FTA1GateError(RuntimeError):
    """The FTA1 report cannot be scored under the frozen contract."""


def score(report: dict[str, object]) -> dict[str, object]:
    if report.get("schema") != "shohin-diverge-fta1-forced-evaluation-v1":
        raise FTA1GateError("FTA1 evaluation schema differs")
    if int(report.get("rows", -1)) != 480:
        raise FTA1GateError("FTA1 gate requires exactly 480 OOD rows")
    ...
    conditions = {
        ...
        "initial_swap_drop_at_least_240": normal["counts"]["terminal_exact"] - initial_swap["counts"]["terminal_exact"] >= 240,
        "operation_shift_drop_at_least_240": normal["counts"]["terminal_exact"] - operation_shift["counts"]["terminal_exact"] >= 240,
    }
    ...
    return {
        "schema": "shohin-diverge-fta1-component-gate-v1",
        ...
        "decision": (
            ...
            if passed
            else "close_fta1_without_autonomous_composition_or_nearby_variants"
        ),
    }

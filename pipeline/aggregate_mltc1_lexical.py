#!/usr/bin/env python3
"""Aggregate the frozen MLTC1 development gate."""
...
class MLTC1AggregateError(ValueError):
    pass
...
def load(path: Path, control: str) -> dict[str, Any]:
    ...
    if payload.get("schema") != "shohin-mltc1-evaluation-v1" or payload.get("status") != "complete" or payload.get("control") != control or payload.get("holdout_used"):
        raise MLTC1AggregateError(f"{control} report differs")
    return payload
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.output.exists():
        raise MLTC1AggregateError("refusing existing output")
    reports = {
        "normal": load(args.normal, "normal"),
        "source_shuffled": load(args.source_shuffled, "source_shuffled"),
        "flat_executor": load(args.flat_executor, "flat_executor"),
        "candidate_state_permuted": load(args.candidate_state_permuted, "candidate_state_permuted"),
    }
    ...
    for report in reports.values():
        if report["checkpoint_sha256"] != reference["checkpoint_sha256"] or report["data_sha256"] != reference["data_sha256"] or report["counts"]["rows"] != 3917:
            raise MLTC1AggregateError("evaluation custody differs")
    normal = reference["rates"]
    family_min = min(value["exact_rate"] for key, value in reference["groups"].items() if key.startswith("family:"))
    source_exact = reports["source_shuffled"]["rates"]["exact_skeleton"]
    flat_loss = group(reference, "hierarchical:true") - group(reports["flat_executor"], "hierarchical:true")
    permuted_loss = normal["exact_skeleton"] - reports["candidate_state_permuted"]["rates"]["exact_skeleton"]
    gates = {
        "lexical_role_sequence_at_least_0p99": normal["lexical_role_sequence_exact"] >= 0.99,
        "selected_lexical_sequence_at_least_0p99": normal["selected_lexical_sequence_exact"] >= 0.99,
        ...
        "source_shuffled_at_most_0p25": source_exact <= 0.25,
        "source_margin_at_least_0p70": normal["exact_skeleton"] - source_exact >= 0.70,
        "flat_hierarchical_loss_at_least_0p35": flat_loss >= 0.35,
        "candidate_state_permutation_loss_at_least_0p35": permuted_loss >= 0.35,
    }
    result = {
        "schema": "shohin-mltc1-comparison-v1", "status": "complete", "holdout_used": False,
        ...
        "source_shuffled_exact": source_exact, "source_causal_margin": normal["exact_skeleton"] - source_exact,
        "flat_hierarchical_loss": flat_loss, "candidate_state_permutation_loss": permuted_loss,
        ...
        "reports": {name: {"path": str(path.resolve()), "sha256": sha256_file(path)} for name, path in {
            ...
            "flat_executor": args.flat_executor, "candidate_state_permuted": args.candidate_state_permuted,
        }.items()},
    }
...
def main() -> int:
    ...
    parser.add_argument("--candidate-state-permuted", type=Path, required=True)

"""Train DIVERGE-QST1 on a pinned product-reasoning backbone."""
...
from diverge_qst1_product import (
    QST1ProductModel,
    frozen_parameter_sha256,
    qst1_architecture_sha256,
)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "workspace_architecture_sha256": qst1_architecture_sha256(
    ...
    while update < args.updates:
        ...
        if update == 1 or update % args.log_interval == 0:
            ...
            event = {
                ...
                "final_halt_probability": metrics["final_halt_probability"],
                "mean_step_delta": metrics["mean_step_delta"],
                "mean_write_gate": metrics["mean_write_gate"],
    ...
    report = {
        "schema": "shohin-diverge-qst1-training-v1",
        "status": "complete",
        **metadata,
        "updates": update,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--binding-temperature", type=float, default=0.10)
    ...
    if any(value <= 0 for value in positive) or args.learning_rate <= 0:
        parser.error("QST1 dimensions and learning rate must be positive")
    if min(args.binding_weight, args.reset_weight, args.halting_weight) < 0:
        parser.error("QST1 loss weights must be nonnegative")
    prefix_slots = args.source_slots + args.state_slots + args.query_slots
    if args.max_sequence_length <= prefix_slots + 16:

#!/usr/bin/env python3
"""Train the frozen FSTC1 fixed-slot skeleton compiler."""
...
SCHEMA = "shohin-fstc1-skeleton-training-v1"


class FSTC1TrainingError(RuntimeError):
    """Raised when FSTC1 custody or training mechanics differ."""
...
def load_programs(path: Path, expected_sha256: str) -> list[TypedProgram]:
    if sha256_file(path) != expected_sha256:
        raise FSTC1TrainingError("training data SHA-256 differs")
    programs = [
        compile_typed_program(json.loads(line))
        for line in path.read_text().splitlines()
        if line.strip()
    ]
    if len(programs) != 75935:
        raise FSTC1TrainingError("training population differs")
    if len({program.identity_sha256 for program in programs}) != len(programs):
        raise FSTC1TrainingError("training identities differ")
    return programs


def batch_order(programs: Sequence[TypedProgram], seed: int) -> list[TypedProgram]:
...
def tokenize_sources(
    tokenizer: Any,
    programs: Sequence[TypedProgram],
    device: torch.device,
    maximum: int,
    ...
    if encoded["input_ids"].shape[1] > maximum:
        raise FSTC1TrainingError("source exceeds frozen token context")
    ...
    for row, program in enumerate(programs):
        for candidate, span in enumerate(program.number_spans):
            ...
            if not candidate_mask[row, candidate].any():
                raise FSTC1TrainingError("numeric span has no tokenizer owner")
    receipt = {
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.output.exists():
        raise FSTC1TrainingError("refusing existing output")
    if args.updates <= 0 or args.batch_size <= 0 or args.gold_feedback_updates < 0:
        raise FSTC1TrainingError("training geometry differs")
    ...
    if not getattr(tokenizer, "is_fast", False):
        raise FSTC1TrainingError("fast tokenizer offsets unavailable")
    ...
    trainable = sum(parameter.numel() for parameter in compiler.parameters())
    if trainable != compiler.parameter_count() or trainable >= 30_000_000:
        raise FSTC1TrainingError("compiler parameter receipt differs")
    ...
    programs = batch_order(
        load_programs(args.data, args.expected_data_sha256), args.data_seed
    )
    cursor = 0
    charged_examples = 0
    ...
    for update in range(1, args.updates + 1):
        if cursor + args.batch_size > len(programs):
            programs = batch_order(programs, args.data_seed + update)
            cursor = 0
        ...
        labels = skeleton_labels(batch, device)
...
def main() -> int:
    ...
    parser.add_argument("--seed", type=int, default=2026081031)
    parser.add_argument("--data-seed", type=int, default=2026081032)

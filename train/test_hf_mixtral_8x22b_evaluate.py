"""CPU tests for the fixed-draft Mixtral-8x22B screen."""
...
import hf_mixtral_8x22b_evaluate as evaluation
...
def _mechanics() -> dict[str, object]:
    return {
        ...
        "model_receipt": {"manifest_sha256": MODEL_MANIFEST_SHA256},
        "score_rows_read": 0,
...
def test_mechanics_report_is_score_free_and_exact(tmp_path: Path) -> None:
    ...
    assert (
        evaluation.validate_mechanics_report(path, MODEL_MANIFEST_SHA256)["status"]
        == "pass"
    )
    payload = _mechanics()
    payload["benchmark_rows_read"] = 1
    path.write_text(json.dumps(payload))
    with pytest.raises(evaluation.MixtralEvaluationError):
        evaluation.validate_mechanics_report(path, MODEL_MANIFEST_SHA256)
...
def test_revision_checkpoint_restore_binds_schedule_and_state(
    ...
    metadata = {
        ...
        "draft_origin_model": evaluation.DRAFT_ORIGIN_MODEL,
        "draft_origin_revision": evaluation.DRAFT_ORIGIN_REVISION,
        "transfer_scope": evaluation.TRANSFER_SCOPE,
    ...
    restored = evaluation.load_revision_checkpoint(path, model, MODEL_MANIFEST_SHA256)
    ...
    with pytest.raises(evaluation.MixtralEvaluationError):
        evaluation.load_revision_checkpoint(path, model, MODEL_MANIFEST_SHA256)

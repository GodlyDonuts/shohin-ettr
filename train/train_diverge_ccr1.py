#!/usr/bin/env python3
"""Train the frozen candidate-relative REFERENT owner for DIVERGE-CCR1."""
...
SCHEMA = "shohin-diverge-ccr1-training-report-v1"
TRAIN_SEED = 2026080623


class CCR1TrainingError(RuntimeError):
    """The frozen CCR1 training contract was violated."""
...
def _load_jsonl(
    ...
    if sha256_path(path) != expected_sha256:
        raise CCR1TrainingError(f"protected data hash differs: {path}")
    rows = []
    with path.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            rows.append(row)
    if len(rows) != 50_000:
        raise CCR1TrainingError(f"protected data row count differs: {path}")
    return rows
...
def _load_checkpoint(
    ...
    if sha256_path(path) != expected_sha256:
        raise CCR1TrainingError(f"protected checkpoint hash differs: {path}")
    checkpoint = torch.load(path, map_location=device, weights_only=False)
    if checkpoint.get("schema") != expected_schema:
        raise CCR1TrainingError(f"protected checkpoint schema differs: {path}")
    return checkpoint
    ...
    *,
    ...
    for start in range(0, len(rows), batch_size):
        ...
        if stage == "EVIDENCE":
            ...
            raise CCR1TrainingError(f"unknown CCR1 stage: {stage}")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("CCR1 frozen training schedule differs")
    ...
    model = CounterfactualReferentMachine(CCR1Config()).to(device)
    ...
    if initial_owner_hashes["WORLD"] != source_checkpoint["model_state_sha256"]:
        raise CCR1TrainingError("CCR1 WORLD owner differs")
    if (
        initial_owner_hashes["NUMERIC_EVIDENCE"]
        != evidence_checkpoint["model_state_sha256"]
    ):
        raise CCR1TrainingError("CCR1 numeric-evidence owner differs")
    ...
    rng = random.Random(args.seed ^ 0x43435231)
    history = []
    source_bytes_seen = 0
    ...
    for update in range(1, args.updates + 1):
        ...
        if not torch.isfinite(gradient_norm):
            raise SystemExit("non-finite CCR1 REFERENT gradient")
        optimizer.step()
    ...
    for owner in ("WORLD", "NUMERIC_EVIDENCE"):
        if final_owner_hashes[owner] != initial_owner_hashes[owner]:
            raise SystemExit(f"CCR1 immutable owner changed: {owner}")
    training_evaluation = {
        "evidence": _evaluate(
            model,
            evidence_rows,

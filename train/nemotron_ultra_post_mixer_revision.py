"""Shohin residual surface for the pinned 550B-A55B Nemotron Ultra host."""
...
ARCHITECTURE = "shohin-nemotron-ultra-shared-post-mixer-v1"


class NemotronUltraRevisionError(RuntimeError):
    """The pinned Ultra residual surface differs."""
...
class PostMixerResidual(nn.Module):
    def __init__(self, base: nn.Module) -> None:
        ...
        try:
            ...
            raise NemotronUltraRevisionError(
                "Ultra native mixer has no parameters"
            ) from error
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if not isinstance(native, torch.Tensor) or native.shape != hidden_states.shape:
            raise NemotronUltraRevisionError(
                "Ultra native mixer output geometry differs"
            )
...
class NemotronUltraRevisionModel(nn.Module):
    def __init__(self, backbone: nn.Module) -> None:
        super().__init__()
        try:
            native_surface = validate_loaded_surface(backbone)
        except Q36UpwardMoEUltraHostError as error:
            raise NemotronUltraRevisionError(str(error)) from error
        if native_surface.get("attachment_surface") != ATTACHMENT_SURFACE:
            raise NemotronUltraRevisionError("Ultra attachment surface differs")
        ...
        for index in CONTROLLED_LAYER_INDICES:
            ...
            if getattr(layer, "block_type", None) != "moe":
                raise NemotronUltraRevisionError("Ultra controlled layer is not MoE")
            block = PostMixerResidual(layer.mixer)
            layer.mixer = block
            blocks.append(block)
        self.blocks = nn.ModuleList(blocks)
        if self.trainable_parameter_count() != TRAINABLE_PARAMETERS_PER_ROLE:
            raise NemotronUltraRevisionError("Ultra trainable parameter count differs")
        if any(
            ...
            raise NemotronUltraRevisionError(
                "Ultra native router or expert is trainable"
            )
    ...
    def trainable_parameter_name_sha256(self) -> str:
        ...
        if len(names) != len(CONTROLLED_LAYER_INDICES) * 2:
            raise NemotronUltraRevisionError("Ultra trainable parameter names differ")
    ...
    def trainable_state(self) -> dict[str, torch.Tensor]:
        ...
        if (
            ...
            raise NemotronUltraRevisionError("Ultra trainable state geometry differs")
        return state

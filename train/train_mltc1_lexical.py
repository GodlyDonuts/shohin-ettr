#!/usr/bin/env python3
"""Train the frozen MLTC1 candidate-local lexical transducer."""
...
SCHEMA = "shohin-mltc1-training-v1"


class MLTC1TrainingError(RuntimeError):
    """Raised when MLTC1 training custody differs."""
...
def load_programs(path: Path, expected_sha256: str, expected_rows: int) -> list[LexicalProgram]:
    if sha256_file(path) != expected_sha256:
        raise MLTC1TrainingError("training data SHA-256 differs")
    programs = [load_lexical_program(json.loads(line)) for line in path.read_text().splitlines() if line.strip()]
    if len(programs) != expected_rows:
        raise MLTC1TrainingError("training population differs")
    return programs


def shuffled(programs: Sequence[LexicalProgram], seed: int) -> list[LexicalProgram]:
    result = list(programs)
    random.Random(seed).shuffle(result)
    return result


def tokenize_sources(tokenizer: Any, programs: Sequence[LexicalProgram], device: torch.device, maximum: int):
    encoded = tokenizer(
        [program.question for program in programs], add_special_tokens=False, padding=True,
        truncation=False, return_offsets_mapping=True, return_tensors="pt",
    )
    offsets = encoded.pop("offset_mapping")
    if encoded["input_ids"].shape[1] > maximum:
        raise MLTC1TrainingError("source exceeds frozen token context")
    masks = torch.zeros(len(programs), MAX_CANDIDATES, encoded["input_ids"].shape[1], dtype=torch.bool)
        ...
        for column, candidate in enumerate(program.candidates):
            for token, (start, end) in enumerate(offsets[row].tolist()):
                if end > candidate.start and start < candidate.end:
                    masks[row, column, token] = True
            if not masks[row, column].any():
                raise MLTC1TrainingError("candidate has no tokenizer owner")
    receipt = {
        "maximum_tokens": int(encoded["attention_mask"].sum(1).max()),
        "charged_source_tokens": int(encoded["attention_mask"].sum()),
    }
    return {key: value.to(device) for key, value in encoded.items()}, masks.to(device), receipt
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.output.exists():
        raise MLTC1TrainingError("refusing existing output")
    ...
    compiler = MonotonicLexicalCompiler(
        source_width, width=args.width, encoder_layers=args.encoder_layers, heads=args.heads
    ).to(device=device, dtype=torch.bfloat16)
    trainable = compiler.parameter_count()
    if trainable >= 20_000_000:
        raise MLTC1TrainingError("compiler parameter receipt differs")
    optimizer = torch.optim.AdamW(
        compiler.parameters(), lr=args.learning_rate, betas=(0.9, 0.95), weight_decay=0.01, fused=True
    )
    ...
    for update in range(1, args.updates + 1):
        ...
        encoded, candidate_mask, token_receipt = tokenize_sources(tokenizer, batch, device, args.max_source_tokens)
        labels = lexical_labels(batch, device)
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            output = compiler(source, candidate_mask, labels["surface"], labels["candidate_count"])
            loss = lexical_loss(output, labels)
        if not torch.isfinite(loss):
            raise MLTC1TrainingError("nonfinite loss")
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        gradient_norm = torch.nn.utils.clip_grad_norm_(compiler.parameters(), 1.0)
        if not torch.isfinite(gradient_norm):
            raise MLTC1TrainingError("nonfinite gradient")
        optimizer.step()
        ...
        if update == 1 or update % args.log_interval == 0 or update == args.updates:
            print(f"update={update}/{args.updates} loss={losses[-1]:.5f} gnorm={float(gradient_norm):.4f}", flush=True)
    ...
    torch.save(
        {
            ...
            "config": {"source_width": source_width, "width": args.width, "encoder_layers": args.encoder_layers, "heads": args.heads},
    ...
    report = {
        ...
        "charged_source_tokens": charged_source_tokens, "maximum_source_tokens": maximum_source_tokens,
...
def main() -> int:
    ...
    parser.add_argument("--seed", type=int, default=2026081051)
    parser.add_argument("--data-seed", type=int, default=2026081052)

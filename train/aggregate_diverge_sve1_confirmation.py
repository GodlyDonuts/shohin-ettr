#!/usr/bin/env python3
"""Aggregate the five conditionally opened SVE1 confirmation reports."""
...
from diverge_sve1_data import CONFIRMATION_SEEDS
from diverge_eal1_data import canonical_sha256
from diverge_eal1_runtime import sha256_path
from eval_diverge_sve1 import SCHEMA as EVALUATION_SCHEMA


SCHEMA = "shohin-diverge-sve1-confirmation-aggregate-v1"


def _load(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text())
    if payload.get("schema") != EVALUATION_SCHEMA:
        raise RuntimeError(f"SVE1 confirmation schema differs: {path}")
    return payload
...
def main() -> None:
    ...
    if args.output.exists():
        raise SystemExit("refusing existing SVE1 confirmation aggregate")
    if sha256_path(args.development) != args.development_sha256:
        raise SystemExit("SVE1 development gate hash differs")
    ...
    if (
        ...
        raise SystemExit("SVE1 development did not admit confirmation")
    if len(args.confirmation) != len(CONFIRMATION_SEEDS):
        raise SystemExit("SVE1 confirmation report count differs")
    ...
    if (
        ...
        raise SystemExit("SVE1 confirmation custody/gate differs")

    arms = {}
    for arm in reports[0]["arms"]:
        evidence_exact = sum(
            int(report["arms"][arm]["evidence_events"]["exact"]) for report in reports
        )
        evidence_total = sum(
            int(report["arms"][arm]["evidence_events"]["total"]) for report in reports
        )
        initial_exact = sum(
            int(report["arms"][arm]["initial_events"]["exact"]) for report in reports
        )
        initial_total = sum(
            int(report["arms"][arm]["initial_events"]["total"]) for report in reports
        )
        state_exact = sum(
        ...
        arms[arm] = {
            ...
            "initial_rate": initial_exact / initial_total,

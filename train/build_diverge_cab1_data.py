#!/usr/bin/env python3
"""Build immutable, lineage-disjoint DIVERGE-CAB1 data."""
...
JRB1_REPORT_SCHEMA = "shohin-diverge-jrb1-data-report-v1"
...
def _atomic_jsonl(path: Path, rows: Iterable[Mapping[str, Any]]) -> tuple[int, str]:
    ...
    if sha256_path(path) != digest.hexdigest():
        raise RuntimeError("CAB1 serialized hash differs")
...
def _load_report(path: Path, expected_sha256: str) -> dict[str, Any]:
    if sha256_path(path) != expected_sha256:
        raise RuntimeError(f"CAB1 prerequisite report hash differs: {path}")
    return json.loads(path.read_text())


def _iter_jsonl(path: Path, expected_sha256: str) -> Iterator[dict[str, Any]]:
    if sha256_path(path) != expected_sha256:
        raise RuntimeError(f"CAB1 prerequisite data hash differs: {path}")
...
def _historical_jrb1(
    ...
    if training_rows != int(training_entry["rows"]):
        raise RuntimeError("CAB1 JRB1 training row receipt differs")
    evaluation_rows = 0
    for label, entries in report["files"].items():
        ...
        if len(rows) != int(entry["rows"]):
            raise RuntimeError("CAB1 JRB1 evaluation row receipt differs")
    ...
    return (
        ...
        identities,
        {
            "jrb1_data_report_sha256": report_sha256,
            "jrb1_identity_sha256": report["identity_sha256"],
            "jrb1_training_rows": training_rows,
            "jrb1_evaluation_episodes": evaluation_rows,
            "jrb1_transitive_overlap_gate": True,
        },
    )
...
def _training_audit() -> tuple[str, set[str], set[str], dict[str, Any]]:
    ...
    for row in _training_rows():
        ...
        names.update(str(value) for value in row["decoy_register_table"])
...
def main() -> None:
    ...
    parser.add_argument("--jrb1-data", type=Path, required=True)
    parser.add_argument("--jrb1-data-report-sha256", required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if args.output.exists():
        raise SystemExit(f"refusing existing CAB1 data output: {args.output}")
    args.output.mkdir(parents=True)

    occupied_sources, occupied_names, occupied_identities, parent = _historical_jrb1(
        args.jrb1_data, args.jrb1_data_report_sha256
    )
    historical_counts = {
    ...
    if (
        ...
        or len(training_names) != TRAIN_ROWS * 5
        ...
        raise SystemExit("CAB1 training regeneration/balance/overlap audit failed")
    ...
    for label, seed in (
        ...
        *((f"confirmation_{seed}", seed) for seed in CONFIRMATION_SEEDS),
        ...
        if (
            ...
            raise SystemExit(f"CAB1 {label} balance/overlap audit failed")
        ...
        if (
            ...
            raise SystemExit(f"CAB1 {label} regeneration failed")
        files[label] = {
            "public": {

#!/usr/bin/env python3
"""Train one matched DIVERGE-OQB1 occurrence-quotient arm."""
...
from diverge_oqb1_data import REPORT_SCHEMA as DATA_REPORT_SCHEMA
from diverge_oqb1_data import TRAIN_ROWS, TRAIN_SEED, validate_training_record
...
REPORT_SCHEMA = "shohin-diverge-oqb1-training-report-v1"
UPDATES = 1_000
BATCH_SIZE = 128
LEARNING_RATE = 0.001
...
def _load_jsonl(path: Path, expected_sha256: str) -> list[dict[str, Any]]:
    if sha256_path(path) != expected_sha256:
        raise RuntimeError("OQB1 training data hash differs")
...
def _load_data_report(
    ...
    if sha256_path(path) != expected_sha256:
        raise RuntimeError("OQB1 data report hash differs")
    ...
    if (
        ...
        raise RuntimeError("OQB1 data report custody differs")
    return report
...
def _atomic_torch(path: Path, payload: Mapping[str, Any]) -> None:
    temporary = path.with_suffix(path.suffix + ".tmp")
    if temporary.exists():
        raise RuntimeError("OQB1 stale checkpoint temporary exists")
...
def _model_records(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return [
        {**row, "registers": row["register_table"], "salt": str(row["serial"])}
        for row in rows
    ]
...
def _mention_logits(
    ...
    *,
    ...
    tensors = tensorize_quotient_sources(
        rows, device, text_key=text_key, mention_count=mention_count, mode=mode
    )
    if tensors[2] is None or tensors[3] is None or not torch.all(tensors[4]):
        raise RuntimeError("OQB1 mention tensorization omitted bounds")
    return model.forward_mentions(tensors[0], tensors[1], tensors[2], tensors[3])
...
def _query_logits(
    ...
    *,
    ...
    tensors = tensorize_quotient_sources(
        rows, device, text_key="query_text", mention_count=None, mode=mode
    )
    if not torch.all(tensors[4]):
        raise RuntimeError("OQB1 query quotient omitted its declared occurrence")
    return model.forward_query(tensors[0], tensors[1])
...
def _sample_accuracy(
    ...
    *,
    ...
    for start in range(0, len(sample), 64):
        ...
        query = _query_logits(model, batch, device, mode=mode).argmax(dim=-1)
...
def main() -> None:
    ...
    parser.add_argument(
        "--arm", choices=("treatment", "broken_quotient"), required=True
    )
    ...
    if args.output.exists():
        raise SystemExit(f"refusing existing OQB1 output: {args.output}")
    if (
        args.updates != UPDATES
        or args.batch_size != BATCH_SIZE
        or not math.isclose(args.learning_rate, LEARNING_RATE)
    ):
        raise SystemExit("OQB1 frozen training schedule differs")
    ...
    if len(rows) != TRAIN_ROWS:
        raise SystemExit("OQB1 training row count differs")
    if not torch.cuda.is_available():
        raise SystemExit("OQB1 training requires CUDA")
    device = torch.device("cuda")
    mode: QuotientMode = "coherent" if args.arm == "treatment" else "broken"

    torch.manual_seed(TRAIN_SEED)
    random.seed(TRAIN_SEED)
    model = OccurrenceQuotientRegisterBinder().to(device)
    ...
    for update in range(1, args.updates + 1):
        ...
        query_logits = _query_logits(model, batch, device, mode=mode)
        ...
        if not torch.isfinite(loss) or not math.isfinite(gradient_norm):
            raise RuntimeError("OQB1 training became nonfinite")
        optimizer.step()
    ...
    model.eval()
    sample = _sample_accuracy(model, rows, device, mode=mode)
    final_state_sha256 = module_state_sha256(model)
    checkpoint = {
        "schema": CHECKPOINT_SCHEMA,
        "source_commit": args.source_commit,
        "arm": args.arm,
        "config": asdict(OccurrenceQuotientBinderConfig()),

#!/usr/bin/env python3
"""Matched plural-family gate for query-valued sparse revision."""
...
from query_valued_revision import (
    QueryValuedRevisionCore,
    ValueArm,
    ValueRevisionTrajectory,
)


SCHEMA = "shohin-qvesr-plural-reasoning-v1"


class ValueRevisionGateError(RuntimeError):
    """The query-valued revision gate violated its fixed contract."""


class ValueRevisionReasoner(nn.Module):
    def __init__(
        self,
        ...
        if board_config.width != revision_config.width:
            raise ValueRevisionGateError("board and revision widths differ")
        self.encoder = EpisodeEncoder(board_config)
        self.core = QueryValuedRevisionCore(revision_config, arm)
...
def final_behavior_loss(
    trajectory: ValueRevisionTrajectory, batch: PluralBatch
) -> torch.Tensor:
...
def metrics(
    logits: torch.Tensor,
    trajectory: ValueRevisionTrajectory,
    batch: PluralBatch,
) -> dict[str, float]:
    ...
    probability = trajectory.steps[-1].utility_probability.clamp_min(1e-9)
    entropy = -(probability * probability.log() * mask).sum(-1).mean()
    result.update(
        {
            "initial_contradiction": first_mean.item(),
            "final_contradiction": last_mean.item(),
            "contradiction_reduction": (first_mean - last_mean).item(),
            "revision_fraction": trajectory.steps[-1].slot_mask.float().mean().item(),
            "correction_rms": trajectory.steps[-1].correction_rms.mean().item(),
...
def _atomic_json(path: Path, payload: dict[str, Any]) -> None:
    if path.exists():
        raise ValueRevisionGateError(f"refusing existing report: {path}")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if not torch.cuda.is_available() and not args.allow_cpu:
        raise ValueRevisionGateError("CUDA is required unless --allow-cpu is explicit")
    ...
    if cohort_manifest.get("schema") != "shohin-fcpt-plural-cohort-v1":
        raise ValueRevisionGateError("cohort manifest schema differs")
    if cohort_manifest.get("status") != "frozen":
        raise ValueRevisionGateError("cohort manifest is not frozen")
    if cohort_manifest.get("config") != asdict(board_config):
        raise ValueRevisionGateError("cohort manifest config differs")
    cohort_hashes = {
        (
            int(row["family_id"]),
            int(row["depth"]),
    ...
    model = ValueRevisionReasoner(board_config, revision_config, args.arm).to(device)
    ...
    for family in range(len(FAMILIES)):
        for depth in (5, 7):
            ...
            if args.arm == "utility":
                for ablation in ("outcomes", "selector_query"):
                    evaluations.append(
                        evaluate(
                            model,
                            family=family,
                            ...
                            shuffle_selector_query=ablation == "selector_query",
                        )
                    )
    observed = {
        (
        ...
        if not row["shuffle_outcomes"] and not row["shuffle_selector_query"]
    }
    if set(observed) != set(cohort_hashes) or any(
        observed[key] != cohort_hashes[key] for key in cohort_hashes
    ):
        raise ValueRevisionGateError("generated development cohort hash differs")
    checkpoint = args.output.with_suffix(".pt")
    if checkpoint.exists():
        raise ValueRevisionGateError(f"refusing existing checkpoint: {checkpoint}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--arm", choices=("utility", "fixed", "residual"), required=True
    )
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--seed", type=int, default=29)
    parser.add_argument("--data-seed", type=int, default=20260880)

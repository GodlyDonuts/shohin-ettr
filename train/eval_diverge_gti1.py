#!/usr/bin/env python3
"""Evaluate one frozen DIVERGE-GTI1 transaction interpreter."""
...
SCHEMA = "shohin-diverge-gti1-evaluation-v1"


class GTI1EvaluationError(RuntimeError):
    """A GTI1 checkpoint, board, or result contract differs."""
...
def _load_model(
    ...
    device: torch.device,
) -> tuple[GenerativeTransactionInterpreter, dict[str, Any]]:
...
def _score(
    ...
    *,
    ...
    return {
        ...
        "score_sha256": hashlib.sha256(scores.contiguous().numpy().tobytes()).hexdigest(),
...
def _conditions(
    ...
    *,
    ...
    return {
        ...
        "mention_swap_equivariance_at_least_765": int(swapped["overall"].get("exact", 0)) >= 765,
        "entity_rename_prompts_exact": prompt_identity_exact,
        "entity_rename_predictions_exact": normal["_predictions"] == renamed["_predictions"],
        "entity_rename_scores_bit_exact": torch.equal(normal["_scores"], renamed["_scores"]),
        "frozen_backbone_bit_exact": frozen_backbone_exact,
        "all_transactions_legal": int(normal["overall"].get("total", 0)) == 768,
    }
...
def main() -> None:
    ...
    if args.output.exists():
        raise SystemExit(f"refusing existing GTI1 result: {args.output}")
    ...
    normal_prompts = [render_prompt(row) for row in records]
    renamed_prompts = [render_prompt(row) for row in renamed_records]
    ...
    report = {
        ...
        "legal_transactions": list(TRANSACTIONS),

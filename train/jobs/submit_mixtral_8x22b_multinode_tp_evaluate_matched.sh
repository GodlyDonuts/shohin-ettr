#!/bin/bash
# Submit one four-rank matched evaluation world behind distributed training.
...
required=(
  ...
  DRAFT_CANDIDATES RUN_ROOT TRAINING_JOB_IDS
)
for variable in "${required[@]}"; do
  [[ -n "${!variable:-}" ]] || { printf '%s is required\n' "$variable" >&2; exit 2; }
done
for path in "$PYTHON" "$RUNTIME" "$MODEL_ROOT" "$MODEL_MANIFEST" \
  "$MECHANICS_REPORT" "$REVISION_CHECKPOINT" "$SOURCE" "$RUN_ROOT"; do
...
IFS=, read -r -a training_jobs <<< "$TRAINING_JOB_IDS"
[[ ${#training_jobs[@]} -eq 4 ]]
dependency="afterok"
...
mkdir -m 700 "$RUN_ROOT/coordination/group_0"
...
exports+=",SHARD_GROUP_INDEX=0,SHARD_GROUP_COUNT=1"
...
printf 'mixtral_multinode_tp_evaluation jobs=%s,%s,%s,%s run_root=%s\n' \

...
required=(
  ...
  CONFIRMATION_ROWS SHARD_COUNT CONFIRMATION_HOST
)
for variable in "${required[@]}"; do
  [[ -n "${!variable:-}" ]] || { printf '%s is required\n' "$variable" >&2; exit 2; }
done
case "$CONFIRMATION_ROWS:$SHARD_COUNT:$CONFIRMATION_HOST" in
  256:4:gpt_oss_120b_confirmation) ;;
  1023:16:gpt_oss_120b_confirmation_1023) ;;
  *) printf 'confirmation launch geometry differs\n' >&2; exit 2 ;;
esac
...
readarray -t prepared < <("$PYTHON" -P -s -B - "$PREPARED_ROOT/receipt.json" "$CONFIRMATION_ROWS" <<'PY'
...
if (
    ...
    or payload.get("rows") != int(sys.argv[2])
...
"$PYTHON" -P -s -B - "$CROSS_HOST_CONTRACT" "$REVISION_MARGIN_THRESHOLD" "$REVISION_RELIABILITY_VETO" "$CONFIRMATION_HOST" <<'PY'
import json
import math
import pathlib
import sys
...
if (
    ...
    or sys.argv[4] not in contract.get("hosts", {})
...
for arm in unchanged revision; do
  for ((shard=0; shard<SHARD_COUNT; shard++)); do
    exports="$gpt_common,ARM=$arm,SHARD_INDEX=$shard,EXPECTED_ROWS=$CONFIRMATION_ROWS,SHARD_COUNT=$SHARD_COUNT,SOURCE=$PREPARED_ROOT/source.jsonl,EXPECTED_SOURCE_SHA256=$source_sha256,OUTPUT_ROOT=$candidate_root,CONFIRMATION_MMLU_PRO=1"
    dependency=()
...
score_exports="PYTHON=$PYTHON,RUNTIME=$RUNTIME,RUNTIME_MANIFEST_SHA256=$RUNTIME_MANIFEST_SHA256,SOURCE=$PREPARED_ROOT/source.jsonl,EXPECTED_SOURCE_SHA256=$source_sha256,EXPECTED_ROWS=$CONFIRMATION_ROWS,EXPECTED_SHARDS=$SHARD_COUNT,ASSESSORS=$ASSESSORS,EXPECTED_ASSESSORS_SHA256=$ASSESSORS_SHA256,CANDIDATE_ROOT=$candidate_root,OUTPUT=$score"
...
"$PYTHON" -P -s -B - "$RUN_ROOT/dispatch.json" "$selector_job" "$score_job" "$analysis_job" "$SHARD_COUNT" "$((SHARD_COUNT * 2))" "$CONFIRMATION_ROWS" "$CONFIRMATION_HOST" "${qwen_jobs[@]}" "${eval_jobs[@]}" <<'PY'
import json, os, pathlib, sys
path = pathlib.Path(sys.argv[1])
qwen_count = int(sys.argv[5])
evaluation_count = int(sys.argv[6])
job_values = sys.argv[9:]
if len(job_values) != qwen_count + evaluation_count:
    raise SystemExit("confirmation dispatch geometry differs")
payload = {
    ...
    "host": sys.argv[8],
    "rows": int(sys.argv[7]),
    "shards_per_arm": qwen_count,
    "qwen_fixed_draft_jobs": [int(value) for value in job_values[:qwen_count]],
    "evaluation_jobs": [int(value) for value in job_values[qwen_count:]],
    "qwen_fixed_draft_job_count": qwen_count,
    "evaluation_job_count": evaluation_count,
    "independent_single_h100_jobs": len(job_values) + 1,

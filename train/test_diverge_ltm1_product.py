"""CPU integration tests for the DIVERGE-LTM1 product path."""
...
def _model() -> LTM1ProductModel:
    return LTM1ProductModel(
        _backbone(),
        lora_layers=1,
        lora_rank=2,
        lora_alpha=4.0,
...
class DivergeLTM1ProductTests(unittest.TestCase):
    def setUp(self) -> None:
        torch.manual_seed(2026080601)

    def test_full_batch_is_finite_and_reaches_lora_and_workspace(self) -> None:
        model = _model()
        ...
        self.assertEqual(len(metrics["prior_selected_nll_rows"]), 2)
        self.assertEqual(metrics["candidate_charged_tokens"], 10.0)
    ...
    def test_generation_uses_one_declared_complete_prefix(self) -> None:
        model = _model().eval()
        ids = torch.tensor([[1, 2, 3]])
        mask = torch.ones_like(ids)
        highest, highest_mask = model.generation_embeddings(ids, mask)
        self.assertEqual(highest.shape, (1, 5, 12))
        self.assertEqual(highest_mask.shape, (1, 5))

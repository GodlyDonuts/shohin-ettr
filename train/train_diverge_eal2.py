#!/usr/bin/env python3
"""Train the identifiable natural temporal reader for DIVERGE-EAL2."""
...
from diverge_eal2_data import TRAIN_ROWS, TRAIN_SEED, validate_training_record
...
REPORT_SCHEMA = "shohin-diverge-eal2-training-report-v1"
UPDATES = 1_000
BATCH_SIZE = 256
LEARNING_RATE = 0.003
...
def _load_jsonl(path: Path, expected_sha256: str) -> list[dict[str, Any]]:
    if sha256_path(path) != expected_sha256:
        raise RuntimeError("EAL2 training data hash differs")
    rows = []
    ...
    if len(rows) != TRAIN_ROWS:
        raise RuntimeError("EAL2 training row count differs")
    return rows
...
def _view(record: dict[str, Any], counterfactual: bool) -> dict[str, Any]:
    return {
        "source_text": record[
            "counterfactual_text" if counterfactual else "source_text"
        ],
        "numeric_role_ids": record[
            "counterfactual_role_ids" if counterfactual else "numeric_role_ids"
        ],
...
def _evaluate_view(
    ...
    *,
    ...
    for start in range(0, len(rows), batch_size):
        ...
        byte_ids, attention, bounds, registers, targets = tensorize_temporal_sources(
            batch, device
        )
        logits = model(byte_ids, attention, bounds)
        for index in range(len(batch)):
            predicted = hard_temporal_assignment(logits[index], registers[index])
...
def main() -> None:
    ...
    if args.output.exists():
        raise SystemExit(f"refusing existing EAL2 output: {args.output}")
    if (
        ...
        raise SystemExit("EAL2 frozen training schedule differs")
    ...
    model = NaturalTemporalReader(TemporalReaderConfig()).to(device)
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=args.learning_rate, betas=(0.9, 0.95), weight_decay=0.01
    )
    generator = random.Random(args.seed ^ 0x45414C32)
    ...
    for update in range(1, args.updates + 1):
        ...
        byte_ids, attention, bounds, _, targets = tensorize_temporal_sources(
            batch, device
        )

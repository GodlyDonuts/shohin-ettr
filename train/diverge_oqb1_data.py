#!/usr/bin/env python3
"""Deterministic occurrence-quotient register data for DIVERGE-OQB1."""
...
TRAIN_SCHEMA = "shohin-diverge-oqb1-training-v1"
PUBLIC_SCHEMA = "shohin-diverge-oqb1-public-v1"
ASSESSOR_SCHEMA = "shohin-diverge-oqb1-assessor-v1"
REPORT_SCHEMA = "shohin-diverge-oqb1-data-report-v1"
TRAIN_SEED = 2026080831
DEVELOPMENT_SEED = 2026080832
CONFIRMATION_SEEDS = (
    2026080833,
    2026080834,
    2026080835,
...
class OQB1DataError(RuntimeError):
    """An occurrence-quotient register record violates its frozen contract."""
...
def _word(namespace: str, seed: int, serial: int, index: int) -> str:
    if namespace not in ("oqb1-train-operation", "oqb1-train-register"):
        raise OQB1DataError("OQB1 opaque-name namespace differs")
    if serial < 0 or index not in (0, 1):
        raise OQB1DataError("OQB1 opaque-name coordinate differs")
...
def table_rotation(seed: int, serial: int) -> int:
    return int(canonical_sha256(["oqb1-table", seed, serial])[-1], 16) % REGISTERS
...
def rotate_table(values: Sequence[str], rotation: int) -> tuple[str, str]:
    ...
    if (
        ...
        raise OQB1DataError("OQB1 table geometry differs")
...
def canonical_to_position(target: int, rotation: int) -> int:
    ...
    if target not in (0, 1) or rotation not in (0, 1):
        raise OQB1DataError("OQB1 target carrier differs")
    return target if rotation == 0 else 1 - target


def build_training_record(serial: int) -> dict[str, Any]:
    rng = random.Random(canonical_sha256(["oqb1-training", TRAIN_SEED, serial]))
    operation = _word("oqb1-train-operation", TRAIN_SEED, serial, 0)
    registers = tuple(
        _word("oqb1-train-register", TRAIN_SEED, serial, index)
...
def validate_training_record(record: Mapping[str, Any]) -> None:
    if (
        ...
        raise OQB1DataError("OQB1 training schema or seed differs")
    ...
    if (
        ...
        raise OQB1DataError("OQB1 training geometry differs")
    payload = dict(record)
    identity = str(payload.pop("identity_sha256"))
    if canonical_sha256(payload) != identity:
        raise OQB1DataError("OQB1 training identity differs")
...
def validate_evaluation_episode(
    ...
    if (
        ...
        raise OQB1DataError("OQB1 evaluation schema differs")
    ...
    if (
        ...
        raise OQB1DataError("OQB1 evaluation geometry differs")
    for episode in (public, assessor):
        payload = dict(episode)
        identity = str(payload.pop("identity_sha256"))
        if canonical_sha256(payload) != identity:
            raise OQB1DataError("OQB1 evaluation identity differs")

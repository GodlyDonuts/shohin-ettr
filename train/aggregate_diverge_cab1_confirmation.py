#!/usr/bin/env python3
"""Aggregate the five conditionally opened CAB1 confirmation reports."""
...
from diverge_cab1_data import CONFIRMATION_SEEDS
from diverge_eal1_data import canonical_sha256
from diverge_eal1_runtime import sha256_path
from eval_diverge_cab1 import SCHEMA as EVALUATION_SCHEMA


SCHEMA = "shohin-diverge-cab1-confirmation-aggregate-v1"


def _load(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text())
    if payload.get("schema") != EVALUATION_SCHEMA:
        raise RuntimeError(f"CAB1 confirmation schema differs: {path}")
    return payload
...
def main() -> None:
    ...
    if args.output.exists():
        raise SystemExit("refusing existing CAB1 confirmation aggregate")
    if sha256_path(args.development) != args.development_sha256:
        raise SystemExit("CAB1 development gate hash differs")
    ...
    if (
        ...
        raise SystemExit("CAB1 development did not admit confirmation")
    if len(args.confirmation) != len(CONFIRMATION_SEEDS):
        raise SystemExit("CAB1 confirmation report count differs")
    ...
    if (
        ...
        raise SystemExit("CAB1 confirmation custody/gate differs")

#!/usr/bin/env python3
"""Evaluate one frozen DIVERGE-CAB1 table-relative register gate."""
...
from diverge_cab1_runtime import load_bus
...
SCHEMA = "shohin-diverge-cab1-evaluation-v1"
...
def _load_report(path: Path, expected_sha256: str) -> dict[str, Any]:
    if sha256_path(path) != expected_sha256:
        raise RuntimeError(f"CAB1 report hash differs: {path}")
...
def _table(values: Sequence[str], reverse: bool) -> tuple[str, str]:
    ...
    if len(table) != 2 or len(set(table)) != 2:
        raise RuntimeError("CAB1 evaluator table differs")
    ...
    *,
    ...
    return {
        ...
        "query": _predict_queries(
            model, query_records, device=device, batch_size=batch_size
        ),
    }
    ...
    *,
    ...
    for visible, hidden in zip(public, assessor, strict=True):
        ...
        if group == "evidence":
            ...
            raise RuntimeError("CAB1 target group differs")
    return output
...
def _execution_score(
    ...
    *,
    ...
    for packet, visible, hidden in zip(packets, public, assessor, strict=True):
        ...
        for transfer, hidden_transfer in zip(
            ...
            if program_id != str(hidden_transfer["program_id"]):
                raise RuntimeError("CAB1 transfer identity differs")
        ...
        for query, hidden_query in zip(
            ...
            if program_id != str(hidden_query["program_id"]):
                raise RuntimeError("CAB1 query identity differs")
    ...
    if program_cursor != len(program_predictions) or query_cursor != len(
        ...
        raise RuntimeError("CAB1 execution cursor differs")
    if tuple(sorted(by_depth)) != tuple(sorted(TRANSFER_DEPTHS)):
        raise RuntimeError("CAB1 transfer depth board differs")
    depth_report = {
...
def main() -> None:
    ...
    if args.output.exists():
        raise SystemExit("refusing existing CAB1 evaluation")
    ...
    if (
        ...
        raise SystemExit("CAB1 parent/training custody differs")
    ...
    if len(public) != DEVELOPMENT_EPISODES or len(assessor) != DEVELOPMENT_EPISODES:
        raise SystemExit("CAB1 evaluation board geometry differs")
    for visible, hidden in zip(public, assessor, strict=True):
    ...
    if len(board_seeds) != 1 or board_serials != set(range(DEVELOPMENT_EPISODES)):
        raise SystemExit("CAB1 board seed/serial geometry differs")
    board_seed = next(iter(board_seeds))
    if int(data_report["split_reports"][args.board_label]["seed"]) != board_seed:
        raise SystemExit("CAB1 board seed receipt differs")
    if not torch.cuda.is_available():
        raise SystemExit("CAB1 evaluation requires CUDA")
    ...
    treatment, treatment_checkpoint = load_bus(
        args.treatment_checkpoint, args.treatment_checkpoint_sha256
    )
    control, control_checkpoint = load_bus(
    ...
    if (
        ...
        raise SystemExit("CAB1 checkpoint/report binding differs")
    ...
    for name, spec in arm_specs.items():
        ...
        owner_hash = hashlib.sha256(
            (
                reader_hash
                + (control_hash if name == "decoy_table_model" else treatment_hash)
            ).encode()
        ).hexdigest()
    ...
    runtime_path = Path(__file__).with_name("diverge_cab1_runtime.py")
    ...
    conditions = {
        ...
        "source_scrub_within_calibrated_chance": arms["source_scrub"][
            "evidence_register"
        ]["exact_rate"]
        <= 0.30
        and arms["source_scrub"]["initial_register"]["exact_rate"] <= 0.35
        and arms["source_scrub"]["query_register"]["rate"] <= 0.55
        and arms["source_scrub"]["execution"]["state_exact_rate"] <= 0.20
        and arms["source_scrub"]["execution"]["answer_exact_rate"] <= 0.35,
        "decoy_model_within_calibrated_chance": arms["decoy_table_model"][
            "evidence_register"
        ]["exact_rate"]
        <= 0.30
        and arms["decoy_table_model"]["initial_register"]["exact_rate"] <= 0.35
        and arms["decoy_table_model"]["query_register"]["rate"] <= 0.55
        and arms["decoy_table_model"]["execution"]["state_exact_rate"] <= 0.20
        and arms["decoy_table_model"]["execution"]["answer_exact_rate"] <= 0.35,

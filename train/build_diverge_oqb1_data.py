#!/usr/bin/env python3
"""Build immutable, lineage-disjoint DIVERGE-OQB1 data."""
...
CAB1_REPORT_SCHEMA = "shohin-diverge-cab1-data-report-v1"
...
def _atomic_jsonl(path: Path, rows: Iterable[Mapping[str, Any]]) -> tuple[int, str]:
    ...
    if sha256_path(path) != digest.hexdigest():
        raise RuntimeError("OQB1 serialized hash differs")
...
def _load_report(path: Path, expected_sha256: str) -> dict[str, Any]:
    if sha256_path(path) != expected_sha256:
        raise RuntimeError(f"OQB1 prerequisite report hash differs: {path}")
    return json.loads(path.read_text())


def _iter_jsonl(path: Path, expected_sha256: str) -> Iterator[dict[str, Any]]:
    if sha256_path(path) != expected_sha256:
        raise RuntimeError(f"OQB1 prerequisite data hash differs: {path}")
...
def _historical_cab1(
    ...
    if training_rows != int(training_entry["rows"]):
        raise RuntimeError("OQB1 CAB1 training row receipt differs")
    evaluation_rows = 0
    for label, entries in report["files"].items():
        ...
        if len(rows) != int(entry["rows"]):
            raise RuntimeError("OQB1 CAB1 evaluation row receipt differs")
    ...
    return (
        ...
        identities,
        {
            "cab1_data_report_sha256": report_sha256,
            "cab1_identity_sha256": report["identity_sha256"],
            "cab1_training_rows": training_rows,
            "cab1_evaluation_episodes": evaluation_rows,
            "cab1_transitive_overlap_gate": True,
        },
    )
...
def main() -> None:
    ...
    parser.add_argument("--cab1-data", type=Path, required=True)
    parser.add_argument("--cab1-data-report-sha256", required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if args.output.exists():
        raise SystemExit(f"refusing existing OQB1 data output: {args.output}")
    args.output.mkdir(parents=True)

    occupied_sources, occupied_names, occupied_identities, parent = _historical_cab1(
        args.cab1_data, args.cab1_data_report_sha256
    )
    historical_counts = {
    ...
    if (
        ...
        raise SystemExit("OQB1 training regeneration/balance/overlap audit failed")
    ...
    for label, seed in (
        ...
        *((f"confirmation_{seed}", seed) for seed in CONFIRMATION_SEEDS),
        ...
        if (
            ...
            raise SystemExit(f"OQB1 {label} balance/overlap audit failed")
        ...
        if (
            ...
            raise SystemExit(f"OQB1 {label} regeneration failed")
        files[label] = {
            "public": {

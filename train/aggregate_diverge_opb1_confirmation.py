#!/usr/bin/env python3
"""Aggregate five fixed DIVERGE-OPB1 confirmation reports."""
...
from diverge_opb1_data import CONFIRMATION_SEEDS
from eval_diverge_opb1 import SCHEMA as EVALUATION_SCHEMA


SCHEMA = "shohin-diverge-opb1-confirmation-aggregate-v1"


def _load(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text())
    if payload.get("schema") != EVALUATION_SCHEMA:
        raise RuntimeError(f"OPB1 confirmation schema differs: {path}")
    return payload
...
def main() -> None:
    ...
    if args.output.exists():
        raise SystemExit("refusing existing OPB1 confirmation aggregate")
    if sha256_path(args.development) != args.development_sha256:
        raise SystemExit("OPB1 development hash differs")
    ...
    if (
        ...
        raise SystemExit("OPB1 confirmation custody/gate differs")

    arms = {}
    for arm in reports[0]["arms"]:
        operation_exact = sum(
            int(report["arms"][arm]["operation"]["exact"]) for report in reports
        )
        operation_total = sum(
            int(report["arms"][arm]["operation"]["total"]) for report in reports
        )
        ...
        arms[arm] = {
            ...
            "operation_rate": operation_exact / operation_total,
            "law_exact": law_exact,
            "law_total": law_total,
